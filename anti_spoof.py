# USAGE
# python anti_spoof.py --model liveness.model --le le.pickle --detector face_detector
        ...
        for i in range(0, detections.shape[2]):
                ...
                if confidence > conf:
                        ...
                        if j==1:
                            if conf<90:
                                if(profile!=None):
                                    label = str(label) + " " + str(preds[j])[:5] + " " + str(profile[0])
                                    label = str(label) + " " + str(preds[j])[:5] + " " + str(profile[1])
                            ...
                            label = "Not Authorised" #label = "{}: {:.4f} {} \n Not Authorised".format(label, preds[j], str(j))
                            cv2.putText(frame, label, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
